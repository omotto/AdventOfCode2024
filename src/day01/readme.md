...
Maybe the lists are only off by a small amount! To find out, pair up the numbers and measure how far apart they are. Pair up the __smallest number in the left list__ with the __smallest number in the right list__, then the __second-smallest left number with the second-smallest right number__, and so on.
...
For these example lists, here is the process of finding the __similarity score__:
